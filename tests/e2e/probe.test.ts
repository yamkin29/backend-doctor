...
test("probe runs the app instrumented: passthrough, warning, session files, exit 0", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
...
test("--duration bounds the session and escalates shutdown", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
...
test("SIGINT to the probe is forwarded, the session finalized, exit 130", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
...
test("usage and environment errors: exit 2, stderr reason, empty stdout, no session", {
	timeout: 30000,
}, async () => {
	const okPath = fixture("ok.js");
...
test("descendant node processes attach with their own pid (NODE_OPTIONS inheritance)", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
	const result = await runCliAsync(
...
test("probe writes findings.json and a stderr summary for a blocking app", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
...
test("probe --filter drives findings at the CLI level", {
	timeout: 30000,
}, async () => {
	const excluded = tmpCwd();
	fs.copyFileSync(fixture("blocking.cjs"), path.join(excluded, "blocking.cjs"));
	...
	fs.copyFileSync(fixture("blocking.cjs"), path.join(included, "blocking.cjs"));
...
test("probe finalizes a zero-state findings.json when the target is not Node", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
	const result = await runCliAsync(
...
test("invalid collector knobs are usage errors before spawn", {
	timeout: 30000,
}, async () => {
...
test("lag windows are recorded in bounded sessions", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
	const result = await runCliAsync(
...
test("probe records an http session: endpoints, summary, unchanged session.json", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
	const result = await runCliAsync(
...
test("probe records prisma queries per request and warns on N+1 via the knob", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
	const result = await runCliAsync(
...
test("invalid BACKEND_DOCTOR_PROBE_N1_THRESHOLD is a usage error before spawn", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
	const result = await runCliAsync(["probe", "--", "node", fixture("ok.js")], {
		cwd,
		env: { BACKEND_DOCTOR_PROBE_N1_THRESHOLD: "abc" },
	});
	expect(result.exitCode).toBe(2);
	expect(result.stdout).toBe("");
...
test("not-Node targets produce structural zero states for the new sections", {
	timeout: 30000,
}, async () => {
	const cwd = tmpCwd();
	const result = await runCliAsync(
